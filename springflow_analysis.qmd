...
p3 <- ggplot(by_gage_month)+geom_point(aes(Month,flow_mean, group = Month))+
  facet_wrap(~gage_name)+
  scale_x_continuous(breaks = seq(0,12,1),
                   minor_breaks = seq(0,12,1))+
  labs(
    title = "Monthly Average Streamflow by Gage",
    subtitle = "Comal and San Marcos Gages",
    x = "Month",
    y = "Mean Flow (cfs)"
  )

p4 <- ggplot(by_gage_month)+geom_point(aes(Month,flow_mean, group = Month, color = gage_name, shape = gage_name))+
  labs(
...
#find way to remove new/old channels and combine comal r+s and san marcos r+s
#add geom_smooth to see if there are any interesting long term trends
```
...
select_streams<- streams|>
  semi_join(sucralose,
            by ="Date")|>
  arrange(Date)
view(select_streams)#arranged in ascending date order to facilitate joining with sucralose data. will need to remove comal new/old channels and determine which cfs measurement to use at each site (average?)
```
